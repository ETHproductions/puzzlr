...
export default class Puzzle {
    ...
    variables = [];
    constraints = [];
    ps = new PartialSolution(this);
    base_partsol = this.ps;
    current_depth = 0;
    partsols_by_depth = [];
    global_stats;
    start_time;
    ...
    debug_log(debug_level, ...args) {
        if (this.options.debug < debug_level + +(this.ps.depth > 0))
            return;
        if (typeof args[0] == "function")
            args = args[0]();
        args[0] =
            this.ps.assumptions
                .map((x) => (x.variable.vpos ? "V" : "S") + x.variable.var_id + "=" + x.value)
                .join(";") +
                " " +
                args[0];
    ...
    format_con(constraint) {
        let varstring = constraint.variables
            ...
            .join(", ");
        if (constraint.variables.length > 5)
            varstring += ", ...";
    ...
    initiate_solve(options) {
        ...
        this.debug_log(0, "Created", this.variables.length, "variables and", this.constraints.length, "constraints");
     ...
     */
    next_step() {
        if ((this.options.mode != "fast" || this.ps.deduct_queue.length == 0) &&
            this.ps.check_queue.length > 0) {
            if (!this.next_check())
                return false;
        }
        else if ((this.options.mode != "thorough" || this.ps.check_queue.length == 0) &&
    ...
    next_check() {
        ...
        for (const value of values) {
            ...
            if (values.every((v) => ++this.global_stats.total_contradiction_checks &&
    ...
    next_deduct() {
        ...
        for (const constraint of variable.constraints)
            for (const subvar of constraint.check.global
                ...
                if (!this.ps.check_queue.some((x) => x.variable.var_id == subvar.var_id &&
    ...
    prepare_next_depth() {
        ...
        if (this.current_depth > 1) {
            ...
            const first_index = parent_partsols.findIndex((ps, i) => ps.assumptions[0].variable.var_id >
        ...
        for (const partsol of parent_partsols) {
            for (const variable of this.variables.slice(this.current_depth == 1
                ? 0
    ...
    run_next_depth() {
        ...
        let success = false;
        for (const ps of current_partsols)
            ps.done = false;
...
class PartialSolution {
    ...
    merge_from_children(assumptions) {
        ...
        if (!valid_children.every((ps) => ps.done || ps.status == "contradiction"))
            return false;
        const shared_assumptions = assumptions.map((a) => valid_children.every((ps) => ps.assumptions.some((b) => a.variable == b.variable && a.value == b.value))
            ? a
        ...
        if (successful_deductions.length > 0) {
            this.debug_log(2, () => [
                ...
                "agreements found in {",
                shared_assumptions
                    .map((a) => this.format_var(a.variable) +
