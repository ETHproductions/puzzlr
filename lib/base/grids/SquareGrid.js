import { PuzzleGrid } from "../index.js";
    ...
    finalize() {
        for (let y = 0; y <= this.height; y++)
            for (let x = 0; x <= this.width; x++) {
                ...
                const cells = (+(this.areamap.get2DUnsafe(x - 1, y - 1, null) != null) << 3) |
                ...
                if (cells & 0b1100) {
                    const newEdge = this.addEdge(this.vertmap.get2D(x, y - 1), newVert, { d: "v", x, y: y - 1 }, false);
                ...
                if (cells & 0b1010) {
                    const newEdge = this.addEdge(this.vertmap.get2D(x - 1, y), newVert, { d: "h", x: x - 1, y }, !(cells & 0b1000));
