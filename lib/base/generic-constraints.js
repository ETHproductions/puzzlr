...
export function CONTIG_EDGE_ALL(edges, target, start) {
    ...
    if (start.value.length == 1 && start.value[0] == target) {
        ...
        if (!edges.some((e) => e.value.length == 1 && e.value[0] == target && e != start))
            return true;
        ...
        while (check_queue.length > 0) {
            ...
            for (const edge of nextVert.edges) {
                ...
                if (found.has(edge))
                    continue;
                if (!edge.value.includes(target))
                    continue;
                if (edge.value.length == 1 && edge.value[0] == target)
                    return true;
                found.add(edge);
...
export function CONTIG_CELL_ALL(cells, target, start) {
    ...
    if (start.value.length == 1 && start.value[0] == target) {
        ...
        if (!cells.some((c) => c.value.length == 1 && c.value[0] == target && c != start))
            return true;
        ...
        while (check_queue.length > 0) {
            ...
            if (found.has(nextCell))
                continue;
            if (!nextCell.value.includes(target))
                continue;
    ...
    // expand alternatingly until a connection is found between the branches.
        ...
        main: while (paths.length > 0) {
            ...
            if (check_queue.length == 0) {
                if ([...found].some((cell) => cell.value.length == 1 && cell.value[0] == target)) {
                    if (found_branch ||
                        paths.some((p) => [...p.found].some((c) => c.value.length == 1 && c.value[0] == target))) {
