...
import { PuzzleVariable, PuzzleVariableValue, PuzzleVariableValues } from "./PuzzleVariable";
...
export default class Puzzle {
     ...
     */
    addConstraint(check: ConstraintCheck, variables: PuzzleVariable[], ...targets: any[]) {
        const constraint = { id: this.constraints.length, check, variables, targets };
    ...
    next_deduct() {
        ...
        for (const constraint of variable.constraints)
            for (const subvar of constraint.check.global ? [variable] : constraint.variables.filter(v => v.value.length > 1))
    ...
    prepare_next_depth() {
        ...
        if (this.current_depth > 1) {
            ...
            const first_index = parent_partsols.findIndex((ps, i) => ps.assumptions[0].variable.var_id > parent_partsols[(i + 1) % parent_partsols.length].assumptions[0].variable.var_id) + 1;
        ...
        for (const partsol of parent_partsols) {
            for (const variable of this.variables.slice(this.current_depth == 1 ? 0 : partsol.assumptions.slice(-1)[0].variable.var_id + 1)) {
                ...
                for (const value of values) {
                    ...
                    if (this.current_depth == 1) {
                        for (const v2 of values) if (v2 != value) {
    ...
    check_if_done() {
        ...
        if (this.variables.every(v => v.value.length == 1 || !v.must_be_unique)) {
            this.debug_log(0, "Puzzle solved in", (Date.now().valueOf() - (this.start_time?.valueOf() ?? 0)) / 1000, "seconds");
...
class PartialSolution {
    ...
    format_var(variable: PuzzleVariable) { return this.puzzle.format_var(variable); }
    format_con(constraint: Constraint) { return this.puzzle.format_con(constraint); }
    ...
    merge_from_children(assumptions: Assumption[]) {
        const missing_variable = assumptions.find(a => !this.assumptions.some(b => a.variable == b.variable))?.variable;
        ...
        const shared_assumptions = assumptions.map(a => valid_children.every(ps => ps.assumptions.some(b => a.variable == b.variable && a.value == b.value)) ? a : { variable: a.variable, value: undefined });
