...
export default class PuzzleGrid {
     ...
     * @returns new vertex
     */
    addVert(rpos: { x: number, y: number }, conns: GridVertex[] | number[] = [], vpos: any = rpos) {
        if (this.#finalized)
     ...
     * 
     * @param fromVert one endpoint of the edge
     * @param toVert other endpoint of the edge
     ...
     * @returns {GridEdge}
     */
    getEdge(fromVert: GridVertex | number, toVert: GridVertex | number): GridEdge {
    ...
    #checkNewCell(fromVert: GridVertex, toVert: GridVertex) {
        ...
        while (i++ < maxSides) {
            ...
            for (const vert of toVert.adjacent) if (vert != fromVert) {
     ...
     */
    angleBetweenEdges(edge1: GridEdge, edge2: GridEdge): number {
     ...
     * @returns {Number} angle in [-pi, pi); straight = 0, cw > 0, ccw < 0
     */
    angleThroughVertex(vert1: GridVertex, pivot: GridVertex, vert2: GridVertex): number {
