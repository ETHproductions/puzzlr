...
function resetPuzzle(newPuzzle: Puzzle, newOptions: any) {
    ...
    for (let groupName of ["cells-base", "edges-base", "hints", "answer", /* "cells-hitbox", "edges-hitbox" */]) {
...
function renderPuzzle() {
    ...
    let edgefuncs = renderfuncs.map(f => renderElements.edge[f]).filter(f => f);
    let cellfuncs = renderfuncs.map(f => renderElements.cell[f]).filter(f => f);

    let render = (objects: PuzzleVariable[], cache: { elems: SVGElement[], value: PuzzleVariableValues }[], funcs: ((v: any) => void)[]) => {
...
const renderElements: {
    ...
    edge: {
        ...
        "edgedraw": (edge) => {
            createSVGElement("line", "edges-base", {
                "stroke": edge.value.length == 1 && edge.value[0] == 0 ? "transparent" : "black",
        ...
        "edgedomino": (edge) => {
            createSVGElement("line", "edges-base", {
                "stroke": edge.isEdgeOfGrid || edge.value.length > 1 || edge.value[0] != 1 ? "black" : "transparent",
                "stroke-width": edge.isEdgeOfGrid ? 3 : edge.value.length == 1 && edge.value[0] == 0 ? 2 : 1,
                "stroke-linecap": "square",
    ...
    cell: {
        ...
        "binarystar": (cell) => {
            createSVGElement("path", "cells-base", cell.value.length == 1 && cell.value[0] == 1 ? {
                "fill": "#333",
                "d": [...Array(10)].map((_, i) => {
                    let { x, y } = cell.midpoint;
                    let angle = i * (Math.PI / 5) - (Math.PI / 2);
                    let r = [0.3, 0.15][i % 2];
                    return (i > 0 ? "L " : "M ") + [convertX(x + Math.cos(angle) * r), convertY(y + Math.sin(angle) * r)]
                }).join(" ") + " Z"
            } : {
        ...
        "sudoku": (cell) => {
            ...
            if (cell.value.length == 1) {
                addHint(cell.value[0].toString(36).toUpperCase(), convertX(x), convertY(y), "#60B", scale / 1.5);
                return;
            }
            ...
            for (let i = 0; i < maxvals; i++) {
                ...
                if (cell.value.includes(i + 1)) {
                    addHint(val, convertX(x - 0.5 + (i % numacross + 0.5) / numacross), convertY(y - 0.5 + ((i / numacross | 0) + 0.5) / numdown), "#60B", fontsize);
                }
            }
        },
        "stitchhole": (cell) => {
        ...
        "binarythermo": (cell: GridCell) => {
            let prevCell = cell.adjacentEdge.find(c => c.area_id == cell.area_id && c.thermoIndex == (cell.thermoIndex ?? 0) - 1);
            let nextCell = cell.adjacentEdge.find(c => c.area_id == cell.area_id && c.thermoIndex == (cell.thermoIndex ?? 0) + 1);
            ...
            createSVGElement("path", "answer", {
                "fill": fill,
                "d": `M ${p[0]} L ${p[1]} A ${tw * scale} ${tw * scale} 0 0 0 ${p[2]} L ${p[3]} L ${p[4]} L ${p[5]} L ${p[6]}`
            });
            createSVGElement("path", "answer", {
                "fill": "transparent",
                "stroke": stroke,
                "stroke-width": swidth,
                "d": `M ${p[0]} L ${p[1]} A ${tw * scale} ${tw * scale} 0 0 0 ${p[2]} L ${p[3]} M ${p[4]} L ${p[5]} L ${p[6]}`
            });
        }
    }
};
